...
for namespace in $APP_DEPLOYMENT_NAMESPACE $PIPELINE_NAMESPACE; do
  create_namespace_read_only_role $namespace
done
...
for namespace in $APP_DEPLOYMENT_NAMESPACE $PIPELINE_NAMESPACE; do
  bind_namespace_role_to_service_account namespace-readonly "$namespace" "$SERVICE_ACCOUNT_NAME" "$APP_DEPLOYMENT_NAMESPACE"
done
