...
function display_pod_distribution() {
  echo "Pod distribution:"
  kubectl get pod -o=custom-columns=NAME:.metadata.name,NAMESPACE:.metadata.namespace,STATUS:.status.phase,NODE:.spec.nodeName --all-namespaces
}

function display_node_details() {
  echo "Node details:"
