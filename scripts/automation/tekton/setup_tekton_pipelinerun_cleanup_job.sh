...
ENVIRONMENT_CONFIG_FILE="${PLATFORM_CONFIG_DIRECTORY}/${ENVIRONMENT}/static.json"

DOCKER_REGISTRY_BASE_URL=$(jq -r '.DOCKER_REGISTRY_BASE_URL' "$ENVIRONMENT_CONFIG_FILE")
TEKTON_CI_IMAGE_NAME=$(jq -r '.TEKTON_CI_IMAGE_NAME' "$ENVIRONMENT_CONFIG_FILE")
TEKTON_CI_IMAGE_TAG=$(jq -r '.TEKTON_CI_IMAGE_TAG' "$ENVIRONMENT_CONFIG_FILE")
CI_IMAGE="$DOCKER_REGISTRY_BASE_URL/${TEKTON_CI_IMAGE_NAME}:${TEKTON_CI_IMAGE_TAG}"

TEAM_ENCRYPTED_CONFIG_FILE="${PLATFORM_CONFIG_DIRECTORY}/${ENVIRONMENT}/${TEAM}/static.encrypted.json"
DOCKER_REGISTRY_AUTH_URL=$(jq -r ".DOCKER_REGISTRY_CREDENTIALS[] | select(.NAME==\"$DOCKER_REGISTRY_SECRET_NAME\").DOCKER_REGISTRY_AUTH_URL" "$TEAM_ENCRYPTED_CONFIG_FILE")
DOCKER_REGISTRY_EMAIL=$(jq -r ".DOCKER_REGISTRY_CREDENTIALS[] | select(.NAME==\"$DOCKER_REGISTRY_SECRET_NAME\").DOCKER_REGISTRY_EMAIL" "$TEAM_ENCRYPTED_CONFIG_FILE")
DOCKER_REGISTRY_USERNAME=$(jq -r ".DOCKER_REGISTRY_CREDENTIALS[] | select(.NAME==\"$DOCKER_REGISTRY_SECRET_NAME\").DOCKER_REGISTRY_USERNAME" "$TEAM_ENCRYPTED_CONFIG_FILE")
DOCKER_REGISTRY_PASSWORD=$(jq -r ".DOCKER_REGISTRY_CREDENTIALS[] | select(.NAME==\"$DOCKER_REGISTRY_SECRET_NAME\").DOCKER_REGISTRY_PASSWORD" "$TEAM_ENCRYPTED_CONFIG_FILE")
